...
class Socket(Serializable):
    ...
    def initSetting(self):
        self.isAddSocket = False

    def getGraphicsSocketClass(self):
        return self.__class__.Socket_GR_Class

    def initInnerClasses(self):
        """Sets up graphics Node (PyQt) and Content Widget"""
        graphics_socket_class = self.getGraphicsSocketClass()
        if graphics_socket_class is not None:
            self.grSocket = graphics_socket_class(self)
    ...
    def codealize(self) -> OrderedDict:
        return OrderedDict([("id", self.id), ("index", self.index),])
